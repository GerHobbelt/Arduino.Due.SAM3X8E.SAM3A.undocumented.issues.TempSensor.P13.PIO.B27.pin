...
PIO_WPSR
  */
  ...
  // testing the dump template lib...
#if 01
  test_display_templates();
  test_display_02();
#endif  
}

// Every loop() run is designed to take 1000 msecs each:
// - 100 msecs spent on repeatedly reading the TS/ADC15 channel and doing some primitive stats on those numbers for funzies.
...
void loop() {
  ...
  if (count == 61) {
    ...
    // reset the stats gathering when the state machine changed state: things change so our old 'tracked average'
    // is dubious and should be discarded. (Not that we're doing important stuff here, just having a bit of fun,
    // wondering about the huge noise coming from our TS on-MCU temp sensor, but anyway...)
    //
    if (state_changed) {
      V_tracked_avg = 0;
      V_noise = 0;
    ...
    while(micros() < t1) {
      ...
      Serial.print(msgbuf << "ADC =" << V << "/ avg.:" << V_tracked_avg << ", dV:" << dV << ", noise:" << V_noise << ", count:" << V_count << "@ cycle-counter:" << count << "@ micros dT=" << (micros() - t0));
    }
